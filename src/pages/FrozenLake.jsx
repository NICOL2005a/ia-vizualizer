...
export default function FrozenLake() {
  ...
  const changeCell = (row, col) => {
    ...
    const currentIndex = CELL_TYPES.indexOf(current);
    const nextType = CELL_TYPES[(currentIndex + 1) % CELL_TYPES.length];

    newBoard[row][col] = nextType;
    setBoard(newBoard);
  };
  ...
  const runBFS = async () => {
    try {
      ...
      const startCount = board.flat().filter((c) => c === "S").length;
      const goalCount = board.flat().filter((c) => c === "G").length;

      if (startCount !== 1) {
        alert("Debe existir exactamente un S");
        return;
      }

      if (goalCount !== 1) {
        alert("Debe existir exactamente un G");
        return;
      }
      ...
      <div style={{ padding: "30px" }}>
        ...
        >
          ...
          >
            ...
            <button style={buttonStyle} onClick={resetSimulation}>
              Reiniciar
            </button>
          </div>
        </div>
        ...
        >
          ...
          <FrozenLakeHistory history={history} />
        </div>
      </div>
    </Layout>
  );
